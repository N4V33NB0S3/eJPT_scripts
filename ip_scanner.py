        ...
        for attempt in range(args.retries):
            ...
            try:
                        ...
                        for port in args.ports:
                            if check_port(ip, port, args.timeout):
                                ...
                                if args.verbose and not args.quiet:
                                    with print_lock:
                                        print(f"\n[+] {ip} is up and port {port} is open")
                            elif args.verbose and not args.quiet:
                                with print_lock:
                                    print(f"\n[+] {ip} is up but port {port} is closed")
                        if open_ports:
                            with list_lock:
                ...
                if attempt == args.retries - 1 and args.verbose and not args.quiet:
                    with print_lock:
                        print(f"\n[-] {ip} is down after {args.retries} attempts")
...
def progress_reporter(total_hosts, args):
    """Report progress on a single line with continuous updates."""
    if args.quiet:
        return
    last_percentage = 0
    while not stop_event.is_set() and tasks_completed < total_hosts:
        ...
        if percentage > last_percentage:
            with print_lock:
                print(f"\rProgress: {percentage:.1f}% ({completed}/{total_hosts} hosts)", end="")
            last_percentage = percentage
        time.sleep(0.1)
    # Ensure 100% is shown and move to new line
    with tasks_lock:
        completed = tasks_completed
    with print_lock:
        print(f"\rProgress: 100.0% ({completed}/{total_hosts} hosts)")
